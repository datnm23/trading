...
from journal.trade_logger import TradeLogger, TradeRecord, JournalEntry, EquitySnapshot
...
class TestTradeLogger:
    ...
    def test_log_trade(self, logger):
        rid = logger.log_trade(TradeRecord(
            symbol="BTC/USDT",
            strategy="EMA-Trend",
            side="long",
            entry_price=64000,
    ...
    def test_get_trades(self, logger):
        logger.log_trade(TradeRecord(symbol="BTC/USDT", side="long", entry_price=60000, exit_price=61000, size=0.01, pnl=100))
        logger.log_trade(TradeRecord(symbol="ETH/USDT", side="long", entry_price=3000, exit_price=3100, size=0.1, pnl=100))
    ...
    def test_trade_summary(self, logger):
        logger.log_trade(TradeRecord(symbol="BTC/USDT", side="long", entry_price=60000, exit_price=61000, size=0.01, pnl=100))
        logger.log_trade(TradeRecord(symbol="BTC/USDT", side="long", entry_price=60000, exit_price=59000, size=0.01, pnl=-100))
    ...
    def test_log_journal(self, logger):
        jid = logger.log_journal(JournalEntry(
            date="2026-04-21",
    ...
    def test_snapshot(self, logger):
        sid = logger.snapshot(EquitySnapshot(
            equity=105000,
            cash=95000,
            open_positions=1,
            open_exposure=10000,
    ...
    def test_emotion_distribution(self, logger):
        logger.log_trade(TradeRecord(symbol="BTC/USDT", side="long", entry_price=60000, exit_price=61000, size=0.01, pnl=100, emotion_before="calm"))
        logger.log_trade(TradeRecord(symbol="BTC/USDT", side="long", entry_price=60000, exit_price=61000, size=0.01, pnl=100, emotion_before="fomo"))
        logger.log_trade(TradeRecord(symbol="BTC/USDT", side="long", entry_price=60000, exit_price=61000, size=0.01, pnl=100, emotion_before="calm"))
