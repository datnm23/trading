...
async def test_get_trades_filter_sub_strategy():
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        r = await client.get("/api/v1/trades?sub_strategy=ema")
        assert r.status_code == 200
        data = r.json()
        assert data["count"] == 1
...
async def test_get_trades_filter_symbol():
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        r = await client.get("/api/v1/trades?symbol=ETH/USDT")
        assert r.status_code == 200
        data = r.json()
        assert data["count"] == 1
...
async def test_get_trades_filter_date_range():
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        r = await client.get("/api/v1/trades?start_date=2026-04-02T00:00:00")
        assert r.status_code == 200
        data = r.json()
        assert data["count"] == 1
...
async def test_get_trades_limit():
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        r = await client.get("/api/v1/trades?limit=1")
        assert r.status_code == 200
        data = r.json()
        assert data["count"] == 1
...
async def test_export_trades_csv():
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        r = await client.get("/api/v1/trades/export?format=csv")
        assert r.status_code == 200
        data = r.json()
        assert "csv" in data
        assert "count" in data
...
async def test_export_trades_json():
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        r = await client.get("/api/v1/trades/export?format=json")
        assert r.status_code == 200
        data = r.json()
        assert "trades" in data
        assert data["count"] == 2
...
async def test_export_trades_filter():
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        r = await client.get("/api/v1/trades/export?format=csv&sub_strategy=breakout")
        assert r.status_code == 200
        data = r.json()
        assert data["count"] == 1
...
async def test_trades_empty_result():
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        r = await client.get("/api/v1/trades?sub_strategy=grid")
        assert r.status_code == 200
        data = r.json()
        assert data["count"] == 0
        assert data["trades"] == []
...
async def test_wiki_search():
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        r = await client.post("/api/v1/wiki/search", json={"query": "trend following", "top_k": 3})
        assert r.status_code == 200
        data = r.json()
...
async def test_wiki_search_empty_query():
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        r = await client.post("/api/v1/wiki/search", json={"query": "", "top_k": 3})
        assert r.status_code == 200
        data = r.json()
