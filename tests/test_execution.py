...
from execution.live_trading import LiveTradingEngine, GraduationGate
...
class TestOrderManager:
    ...
    def test_validation_failure(self):
        om = OrderManager(paper_mode=True, validator=OrderValidator(min_order_size=100000))
        order = om.create_order(
            symbol="BTC/USDT",
            side=OrderSide.BUY,
            amount=0.0001,
...
class TestHealthServer:
    ...
    def test_metrics_endpoint(self):
        server = HealthServer(
            port=18082,
            status_provider=lambda: {"running": True, "equity": 100000, "capital": 95000, "open_positions": 1},
        )
        server.start()
        import requests
