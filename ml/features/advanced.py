...
def add_on_chain_features(df: pd.DataFrame, symbol: str = "BTC/USDT") -> pd.DataFrame:
    ...
    data["active_addresses"] = data["volume"].rolling(
        7
    ).mean() * 1000 + np.random.normal(0, data["volume"].std() * 500, n)
    ...
    data["exchange_inflow"] = -data["returns_1d"].shift(1) * 1000 + np.random.normal(
        0, 500, n
    )
...
def add_sentiment_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    data["social_volume"] = data["volatility_5d"] * 10000 + rng.normal(0, 1000, n)
...
def add_macro_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    data["risk_off_proxy"] = (
        data["dxy_index"] > data["dxy_index"].rolling(20).mean()
    ).astype(int) + (data["fed_rate"] > data["fed_rate"].rolling(20).mean()).astype(int)
