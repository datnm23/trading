...
def _get_classifier(
    n_estimators: int = 200, max_depth: int = 5, learning_rate: float = 0.05
):
...
class MLClassifierPipeline:
    ...
    def train(self, df: pd.DataFrame) -> dict:
        ...
        target_col = (
            f"target_direction_{self.target_horizon}d"
            if self.target_horizon != 5
            else "target_direction"
        )
        target_return_col = (
            f"target_return_{self.target_horizon}d"
            if self.target_horizon != 5
            else "target_return_5d"
        )
        ...
        logger.info(
            f"Training complete. Mean accuracy: {self.metrics['mean_accuracy']:.3f}"
        )
        return self.metrics
    ...
    def predict(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        features_df["signal"] = np.where(
            proba > self.threshold, 1, np.where(proba < 1 - self.threshold, -1, 0)
        )
        return features_df
