...
class PsychologicalEnforcer:
    ...
    def __init__(self, config: dict | None = None):
        ...
        self.revenge_emotions = set(
            cfg.get("revenge_emotions", ["revenge", "fomo", "panic", "angry"])
        )
    ...
    def check_state(
        self, pnl: float = 0.0, emotion: str | None = None
    ) -> PsychologicalState:
    ...
    def _apply_rules(self, last_pnl: float):
        ...
        if self.state.consecutive_losses >= self.max_consecutive_losses:
            if not self.state.is_paused:
                ...
                self.state.pause_until = datetime.now() + timedelta(
                    hours=self.pause_after_losses_hours
                )
        ...
        if last_pnl < 0 and self._recent_pnls:
            avg_loss = abs(
                sum(p for p in self._recent_pnls if p < 0)
                / max(1, sum(1 for p in self._recent_pnls if p < 0))
            )
    ...
    def get_summary(self) -> dict:
        ...
        return {
            ...
            "pause_until": (
                self.state.pause_until.isoformat() if self.state.pause_until else None
            ),
