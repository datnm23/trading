...
def parse_concept_file(filepath: Path) -> dict:
    ...
    return {
        ...
        "related_count": int(metadata.get("related_count", 0)) if metadata.get("related_count") else 0,
        "summary": summary,
    ...
    if hubs_dir.exists():
        for hub_file in sorted(hubs_dir.glob("*.md")):
            try:
                ...
                body = content.split("---", 2)[2] if len(content.split("---")) >= 3 else ""
                linked_concepts = re.findall(r'\[([^\]]+)\]\(\.\./concepts/([^\)]+)\)', body)
                hubs.append({
                    "id": hub_file.stem,
